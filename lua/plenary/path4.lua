...
function Path:new(...)
  ...
  setmetatable(obj, {
    ...
    __newindex = function(t, k, val)
      if k == "filename" then
        proxy.filename, proxy._name = val, normalize_path(val)
        proxy._absolute, proxy._fs_stat = nil, nil
      elseif k == "_name" then
        proxy.filename, proxy._name = (val:gsub("/", t._sep)), val
        proxy._absolute, proxy._fs_stat = nil, nil
      else
        proxy[k] = val
      end
...
--- Make an absolute path relative to another path.
---
...
function Path:make_relative(cwd)
  ...
  --   self._name = self._name:sub(#cwd + 1, -1)
  -- else
  --   error(string.format("'%s' is not a subpath of '%s'", self.filename, cwd))
  -- end
  return self.filename
end
...
-- local p = Path:new "C:/Windows/temp/lua/plenary/path.lua"
-- print(p.filename, p:normalize "C:/Windows/temp/lua")

-- local p = Path:new "../lua/plenary/path.lua"
-- print(p:normalize("C:/Windows/temp/lua"))
